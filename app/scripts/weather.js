(function(){
	...
	w1.controller('weatherController', ['$scope', '$http', '$interval', '$filter', function($scope, $http, $interval, $filter	){
		...
		$scope.weatherTime.temp = 'templates/weatherWidget/nowWeather.html';
		...
		$scope.nowFutureToggle = function(){
			...
			$scope.weatherTime.temp = 'templates/weatherWidget/' + temp + '.html';
		};
		...
		$scope.showFutureWeather = function(value) {
			...
			$scope.icon.whereFuture = 'templates/weatherIconSVG/' + iconName + '.html';
		...
		function getNowData(callback) {
			...
			success(function(data, status, headers, config) {
		    	...
		    	$scope.icon.whereNow = 'templates/weatherIconSVG/' + iconName + '.html';
		  	}).
